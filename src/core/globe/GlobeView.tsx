...
import { useNexusGlobeLayer } from "./useNexusGlobeLayer";
...
export default function GlobeView() {
    ...
    // ── NEXUS Globe Layer (alerts + damage zones)
    useNexusGlobeLayer(viewerRef.current, viewerReady);
    ...
    // ── NEXUS entity click → open alert detail panel
    const setSelectedAlert = useStore(s => s.setNexusSelectedAlert);
    const openNexusPanel   = useStore(s => s.toggleNexusPanel);
    const nexusPanelOpen   = useStore(s => s.nexusPanelOpen);
    const setActiveTab     = useStore(s => s.setNexusActiveTab);
    ...
    useEffect(() => {
        ...
        handler.setInputAction((click: { position: any }) => {
            ...
            // NEXUS entities are prefixed __nexus_
            if (!entityId.startsWith("__nexus_")) return;

            // Extract alert ID from entity ID patterns:
            // __nexus_zone_{alertId}, __nexus_ring_{alertId}, __nexus_label_{alertId}
            // __nexus_jam_{alertId}, __nexus_void_{alertId}
            // __nexus_dmg_*_{zoneId}
            const nexusParts = entityId.replace("__nexus_", "").split("_");
            const prefix = nexusParts[0]; // zone, ring, label, jam, void, dmg

            if (prefix === "dmg") {
              // Damage zone click — extract zone name and show info
              const zoneName = nexusParts.slice(2).join("_");
              // For now just fly-to — could open damage zone detail
              return;
            }

            // Alert entity
            const alertId = nexusParts.slice(1).join("_");
            if (alertId) {
                setSelectedAlert(alertId);
                setActiveTab("alerts");
                if (!nexusPanelOpen) openNexusPanel();
                // Expose for TelegramIntelPanel fly-to
                (window as any).__nexusSelectedAlertId = alertId;
            }
        }, ScreenSpaceEventType.LEFT_CLICK);

        return () => { handler.destroy(); };
    }, [viewerReady, setSelectedAlert, openNexusPanel, nexusPanelOpen, setActiveTab]);
    ...
    const ctrl = viewer.scene.screenSpaceCameraController;
        ...
        // Expose flyTo globally for EventDetailPanel
        (window as any).__nexusFlyTo = (lat: number, lng: number, alt = 500000) => {
            viewer.camera.flyTo({
