...
export async function POST(req: NextRequest) {
  ...
  // ── 2. Mise à jour du cluster événement ─────────────────────
  ...
  return NextResponse.json({ ok: true, id: enriched.id, score: enriched.confidence_score });
}
...
function findNearestDamageZone(zoneName: string | null): string | null {
  ...
  const match = DAMAGE_ZONES.find(dz => 
    ...
    zoneName.toLowerCase().includes(dz.country.toLowerCase())
  );
